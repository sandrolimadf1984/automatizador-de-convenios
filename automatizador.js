(() => {
    ...
    if (document.getElementById('menu-automatizador')) return;

    // ── estilos globais do app ────────────────────────────────────
    if (!document.getElementById('estilo-automatizador')) {
        const est = document.createElement('style');
        est.id = 'estilo-automatizador';
        est.textContent =
            '#adc-lista::-webkit-scrollbar{width:9px;}' +
            '#adc-lista::-webkit-scrollbar-track{background:#0a1020;border-radius:5px;}' +
            '#adc-lista::-webkit-scrollbar-thumb{background:linear-gradient(#4dc3ff,#2d7dff);border-radius:5px;}' +
            '#adc-txt-codigos::-webkit-scrollbar{width:8px;}' +
            '#adc-txt-codigos::-webkit-scrollbar-thumb{background:#2d7dff;border-radius:4px;}' +
            '#adc-txt-codigos::placeholder{color:#5b7ba6;text-align:center;padding-top:52px;font-size:13px;}' +
            '.adc-card:hover{border-color:#2d7dff !important;box-shadow:0 0 14px rgba(45,125,255,0.35);}' +
            '.adc-card:hover .adc-seta{background:#2d7dff;color:#fff;}' +
            '#adc-lista,#adc-home,#adc-janela{cursor:default;}' +
            '#adc-home,#adc-janela,#adc-lista{overflow-x:hidden;}' +
            '#adc-home > *{flex:0 0 auto;}' +
            '#adc-janela::-webkit-scrollbar{width:8px;}' +
            '#adc-janela::-webkit-scrollbar-track{background:#0a1020;}' +
            '#adc-janela::-webkit-scrollbar-thumb{background:#2d7dff;border-radius:4px;}' +
            '.adc-rz{position:absolute;z-index:20;}' +
            '.adc-rz-n{top:0;left:9px;right:9px;height:9px;cursor:ns-resize;}' +
            '.adc-rz-s{bottom:0;left:9px;right:9px;height:9px;cursor:ns-resize;}' +
            '.adc-rz-w{left:0;top:9px;bottom:9px;width:9px;cursor:ew-resize;}' +
            '.adc-rz-e{right:0;top:9px;bottom:9px;width:9px;cursor:ew-resize;}' +
            '.adc-rz-nw{top:0;left:0;width:14px;height:14px;cursor:nwse-resize;z-index:21;}' +
            '.adc-rz-se{bottom:0;right:0;width:14px;height:14px;cursor:nwse-resize;z-index:21;}' +
            '.adc-rz-ne{top:0;right:0;width:14px;height:14px;cursor:nesw-resize;z-index:21;}' +
            '.adc-rz-sw{bottom:0;left:0;width:14px;height:14px;cursor:nesw-resize;z-index:21;}' +
            '.adc-rz:hover{background:rgba(45,125,255,0.30);border-radius:4px;}' +
            '.adc-arrasta{cursor:move;}';
        document.head.appendChild(est);
    }

    // ── casca do app ──────────────────────────────────────────────
    const menu = document.createElement('div');
    menu.id = 'menu-automatizador';
    menu.style.cssText = `
        position: fixed;
        top: 16px;
        left: 16px;
    ...
    telaHome.innerHTML = `
        ...
        <div class="adc-arrasta" style="text-align:center;margin:6px 0 12px;font-size:12px;letter-spacing:3px;color:#9db4d8;font-weight:600;">
        ...
        <div style="background:linear-gradient(135deg,#0d1f3a,#0b1830);border:1px solid #24559b;border-radius:12px;padding:14px 16px;margin-bottom:14px;background-image:repeating-linear-gradient(0deg,rgba(45,125,255,0.05) 0 1px,transparent 1px 22px),repeating-linear-gradient(90deg,rgba(45,125,255,0.05) 0 1px,transparent 1px 22px);">
            ...
            <div style="position:relative;">
                <input id="adc-busca" type="text" autocomplete="off" spellcheck="false" placeholder="Digite o nome do convênio..."
                ...
                <div id="adc-busca-limpar" title="Limpar" style="display:none;position:absolute;right:9px;top:50%;transform:translateY(-50%);
        ...
        <div id="adc-vazio" style="display:none;text-align:center;color:#8fa8cf;font-size:11.5px;padding:18px 10px;line-height:1.6;">
            Nenhum convênio encontrado.<br><span style="color:#5f7aa3;font-size:10.5px;">Apague a pesquisa para ver todos.</span>
        </div>

        <div id="adc-lista" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;overflow-y:auto;padding-right:4px;flex:1 1 auto;min-height:70px;align-content:start;"></div>

        <div style="display:flex;align-items:flex-start;justify-content:center;gap:56px;margin:16px 0 6px;">
            <div id="adc-fechar-app" style="text-align:center;cursor:pointer;">
            ...
            <div id="adc-marcador" style="text-align:center;cursor:pointer;">
                <div id="adc-marcador-icone" style="width:52px;height:52px;margin:0 auto;border-radius:50%;border:2px solid #17a2b8;background:radial-gradient(circle at 35% 30%, #0a1b26, #06121c);display:flex;align-items:center;justify-content:center;font-size:24px;color:#4dc3ff;box-shadow:0 0 12px rgba(23,162,184,0.5);transition:all .25s ease;">☐</div>
                <div id="adc-marcador-label" style="margin-top:6px;font-size:9.5px;font-weight:700;letter-spacing:.5px;color:#4dc3ff;">marcador de checkboxes</div>
            </div>
        </div>
    ...
    telaJanela.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
            <div id="adc-voltar" style="width:34px;height:34px;border-radius:50%;background:#0e1a2e;border:1px solid #223a5e;display:flex;align-items:center;justify-content:center;font-size:16px;color:#cfe0ff;cursor:pointer;flex-shrink:0;">←</div>
            <div id="adc-j-icone" style="width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;background:linear-gradient(135deg,#2d7dff,#1a5bcc);box-shadow:0 0 12px rgba(45,125,255,0.5);">📝</div>
            <div style="flex:1;min-width:0;">
                <div id="adc-j-nome" style="font-size:16px;font-weight:800;color:#eaf3ff;letter-spacing:.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">CONVÊNIO</div>
                <div id="adc-j-desc" style="font-size:10.5px;color:#8fa8cf;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Automação</div>
            </div>
            <div id="adc-j-fechar" style="display:flex;align-items:center;gap:6px;background:#2a0f14;border:1px solid #b91f16;border-radius:16px;padding:6px 12px;font-size:10px;font-weight:800;color:#ff6b5e;letter-spacing:1px;cursor:pointer;flex-shrink:0;">✖ FECHAR</div>
        </div>
        ...
        <div style="background:#0c1830;border:1px solid #1e3a66;border-radius:14px;padding:14px;">
            ...
            <textarea id="adc-txt-codigos" placeholder="Clique aqui e cole os códigos do convênio" style="display:block;width:100%;min-height:150px;box-sizing:border-box;background:#0a1526;border:2px dashed #2d7dff;border-radius:12px;padding:12px;color:#cfe4ff;font-size:12.5px;font-family:Consolas,monospace;resize:vertical;outline:none;"></textarea>
        ...
        <button id="adc-iniciar" style="display:block;width:100%;margin-top:14px;padding:14px;background:linear-gradient(180deg,#2d7dff,#1a5bcc);color:#fff;border:1px solid #4dc3ff;border-radius:12px;cursor:pointer;font-weight:800;font-size:14px;letter-spacing:1.5px;box-shadow:0 0 16px rgba(45,125,255,0.5);font-family:inherit;">🚀 INICIAR AUTOMAÇÃO</button>
        <button id="adc-limpar" style="display:block;width:100%;margin-top:8px;padding:11px;background:#0e1a2e;color:#9db4d8;border:1px solid #223a5e;border-radius:12px;cursor:pointer;font-weight:700;font-size:12px;letter-spacing:1.5px;font-family:inherit;">🕐 LIMPAR</button>
        <div id="adc-exec-status" style="display:none;margin-top:10px;background:#0a1424;border:1px solid #1b3157;border-radius:10px;padding:10px 12px;font-size:11px;color:#9db4d8;line-height:1.5;text-align:center;min-height:16px;white-space:pre-line;"></div>
        <div id="adc-motor-info" style="margin-top:6px;font-size:9.5px;color:#3d5a85;text-align:center;"></div>
    `;
    ...
    ['n','s','w','e','nw','ne','sw','se'].forEach(dir => {
        ...
        alca.className = 'adc-rz adc-rz-' + dir;
    ...
    (() => {
        ...
        const iniciar = ev => {
            const alca = ev.target.closest('.adc-rz');
            if (!alca) return;
            const p = ev.touches ? ev.touches[0] : ev;
    ...
    document.getElementById('adc-marcador').onclick = () => {
        let marcados = 0;
        document.querySelectorAll('input[type=checkbox]').forEach(cb => {
            if (!cb.checked && !menu.contains(cb)) { cb.click(); marcados++; }
        });
        const ic = document.getElementById('adc-marcador-icone');
        const lb = document.getElementById('adc-marcador-label');
        ic.innerText = '☑';
    ...
    // ── FECHAR APP ────────────────────────────────────────────────
    document.getElementById('adc-fechar-app').onclick = () => {
        limparPaineisDoRobo();   // não deixa painel escondido para trás na página
        menu.remove();
    };

    // ── SISTEMA DE AVISOS DINÂMICOS ───────────────────────────────
    const linkDoAviso = "https://raw.githubusercontent.com/sandrolimadf1984/automatizador-de-convenios/main/aviso.txt";
    fetch(linkDoAviso + "?t=" + new Date().getTime())
        ...
        .then(texto => {
            if (texto.trim() !== "") {
                ...
                `;
                document.getElementById('adc-aviso-slot').appendChild(avisoDiv);
            }
        })
    ...
    (() => {
        try {
            window.__adcJanelas = window.__adcJanelas || [];
            if (window.__adcHookOpen) return;
            const abrirOriginal = window.open;
            window.open = function () {
                const w = abrirOriginal.apply(window, arguments);
                try { if (w) window.__adcJanelas.push(w); } catch (e) { }
                return w;
            };
            window.__adcHookOpen = true;
        } catch (e) { }
    })();
    ...
    const acharJanelaSADT = () => {
        ...
        const lista = (window.__adcJanelas || []).slice().reverse();
    ...
    const robos = {
        ...
        "TJDF": () => {
            (() => {
                if (document.getElementById('b403-painel-box')) return;
                let codigos = [];
                let idx = 0;
                let observer;
                ...
                const foraDoPortal = el => !!(el.closest('#b403-painel-box') ||
                    el.closest('#menu-automatizador') || el.closest('#result-body-table'));
                ...
                const textoDaListaContem = cod => {
                    ...
                    const pular = ['#result-body-table', '#b403-painel-box', '#menu-automatizador']
        ...
        "ASSEDF": () => {
            (function () {
                ...
                var vigia = relogioProprio(function () {
                    ...
                    try { (window.__adcJanelas || []).forEach(function (w) { lista.push(w); }); } catch (e) { }
        ...
        "PROASA_CNU_DESATIVADO": () => {
            (function () {
                ...
                try {
                    (window.__adcJanelas || []).forEach(function (w) {
        ...
        "SAMP": () => {
            (() => {
                if (document.getElementById('samp-painel-box')) return;
                ...
                const camposVisiveis = () => {
                    ...
                    return Array.prototype.slice.call(document.querySelectorAll('input')).filter(i => {
                        ...
                        if (i.closest('#samp-painel-box')) return false;
                        if (i.closest('#menu-automatizador')) return false;
                        return visivel(i);
                    });
                };
    ...
    const btnIniciar = document.getElementById('adc-iniciar');
    const btnLimpar = document.getElementById('adc-limpar');
    const campoCodigos = document.getElementById('adc-txt-codigos');
    const statusExec = document.getElementById('adc-exec-status');
    ...
    const iniciarPadrao = (nome, texto) => {
        ...
        const vigia = setInterval(() => {
            ...
            Array.from(document.body.children).forEach(el => {
                if (el === menu || el.id === 'adc-espelho') return;
    ...
    const abrirJanelaCodigos = item => {
        ...
        document.getElementById('adc-j-nome').innerText = item.rotulo;
        document.getElementById('adc-j-desc').innerText = item.desc;
        const ic = document.getElementById('adc-j-icone');
        ...
        const infoMotor = document.getElementById('adc-motor-info');
    ...
    document.getElementById('adc-voltar').onclick = () => {
        telaJanela.style.display = 'none';
        telaHome.style.display = 'flex';
    };
    document.getElementById('adc-j-fechar').onclick = () => { encerrarModoAutomacao(); menu.remove(); };
    btnLimpar.onclick = () => {
        campoCodigos.value = '';
        campoCodigos.focus();
    };
    ...
    const marcaHtml = (rotulo, icone, cor, tam) => {
        ...
        if (!logo) {
            return '<div class="adc-marca" style="width:' + tam + 'px;height:' + tam + 'px;border-radius:10px;flex-shrink:0;' +
    ...
    const reservaDaMarca = (raiz, icone, cor) => {
        const img = raiz.querySelector('.adc-marca img');
        if (!img) return;
        img.onerror = () => {
    ...
    const container = document.getElementById('adc-lista');
    for (const item of EXIBICAO) {
        ...
        card.innerHTML = `
            <div style="display:flex;align-items:flex-start;gap:9px;">
                ...
                <div class="adc-seta" style="width:22px;height:22px;border-radius:50%;flex-shrink:0;background:#0e1a2e;border:1px solid #223a5e;display:flex;align-items:center;justify-content:center;font-size:11px;color:#cfe0ff;transition:all .15s ease;align-self:center;">›</div>
            </div>
    ...
    (function () {
        const campo = document.getElementById('adc-busca');
        const limpar = document.getElementById('adc-busca-limpar');
        const vazio = document.getElementById('adc-vazio');
        if (!campo) return;
        ...
        const filtrar = () => {
            ...
            let achou = 0;
            const cards = container.querySelectorAll('.adc-card');
        ...
        campo.addEventListener('keydown', e => {
            ...
            if (e.key === 'Enter') {
                const visiveis = [...container.querySelectorAll('.adc-card')].filter(c => c.style.display !== 'none');
